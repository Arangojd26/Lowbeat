...
const CardLogin = (props) => {
    ...
    const [verIconViewPass, setVerIconViewPass] = React.useState("iconpass d-none")
    const [verIconViewPassConfirm, setVerIconViewPassConfirm] = React.useState("iconpass d-none")
    const [changeIconPass, setChangeIconPass] = React.useState(false)
    const [changeIconPassConfirm, setChangeIconPassConfirm] = React.useState(false)
    ...
    const controlAnimacionLabelNombre = (val) => val === "" ? setLabelNombre(false) : setLabelNombre(true)
    const controlAnimacionLabelEmail = (val) => val === "" ? setLabelEmail(false) : setLabelEmail(true)
                ...
                <div className="card-body">
                    ...
                    <div className="container-form">
                        <form onSubmit={procesarDatos}>
                            ...
                            {
                                esRegistro ? (
                                    <div className={labelNombre ? "input-group pt-4 focused" : "input-group pt-4"}>
                            ...
                            <div className={labelEmail ? "input-group pt-4 focused" : "input-group pt-4"}>
                                <label className="form-label" htmlFor="first">Correo electrónico</label>
                                <input
                                    type="text"
                                    className="form-control input-login"
                            ...
                            {errorUser && <p className="text-danger text-left ml-3 pt-1"><small>{errorUser}</small></p>}
                            {
                                esRegistro ? (
                                    <>
                                        <div className={labelPass ? "input-group pt-4 focused" : "input-group pt-4"}>

                                            <label className="form-label" htmlFor="last">Contraseña</label>
                                            <input
                                            ...
                                            <span className="input-group-append">
                                                <div className="input-group-text bg-transparent">
                                                    <img className={verIconViewPass} src={changeIconPass ? iconPassShow : iconPassHide} width="18" height="18" alt=""
                                                        onClick={() => setChangeIconPass(!changeIconPass)}
                                                    />
                                                </div>
                                            </span>
                                        </div>
                                        <div className={LabelPassConfirm ? "input-group pt-4 focused" : "input-group pt-4"}>

                                            <label className="form-label" htmlFor="last">Confirmar contraseña</label>
                                            <input
                                            ...
                                            <span className="input-group-append">
                                                <div className="input-group-text bg-transparent">
                                                    <img className={verIconViewPassConfirm} src={changeIconPassConfirm ? iconPassShow : iconPassHide} width="18" height="18" alt=""
                                                        onClick={() => setChangeIconPassConfirm(!changeIconPassConfirm)}
                                                    />
                                                </div>
                                            </span>
                                        </div>
                            ...
                            {error && <p className="text-danger text-left ml-3 pt-2"><small>{error}</small></p>}
                            <SwitchLogin esRegistro={esRegistro} setEsRegistro={setEsRegistro} />
                            <ButtonLogin esRegistro={esRegistro} loading={loading} />
                            <ButtonGoogle setOpenLoading={props.setOpenLoading} loading={loading} />
                        </form>
                    </div>
                </div>
            </div>
